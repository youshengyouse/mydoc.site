...
class AnswerSession {
    ...
    regenerateLast({ stream = true }) {
        ...
        if (!isLastMessageAssistant) {
            throw (0, errors_js_1.createError)('ANSWER_SESSION_LAST_MESSAGE_IS_NOT_ASSISTANT');
        }
        this.messages.pop();
        this.state.pop();
        if (stream) {
    ...
    async *fetchAnswer(params) {
        if (!this.chatModel) {
            throw (0, errors_js_1.createError)('PLUGIN_SECURE_PROXY_MISSING_CHAT_MODEL');
        }
        ...
        this.triggerStateChange();
        try {
            const sources = await (0, search_js_1.search)(this.db, params);
    ...
    async init() {
        ...
        if (!plugin) {
            throw (0, errors_js_1.createError)('PLUGIN_SECURE_PROXY_NOT_FOUND');
        }
